...
function getMealHighlights(mealType) {
  const highlights = {
    breakfast: ['Fresh Pastries', 'Eggs Benedict', 'Smoothie Bowls', 'Artisan Coffee'],
    lunch: ['Signature Salads', 'Gourmet Sandwiches', 'Daily Specials', 'Fresh Juice'],
    dinner: ['Chef\'s Tasting Menu', 'Grilled Specialties', 'Seafood Platters', 'Dessert Selection']
...
function getRestaurantForActivity(destination, mealType, budget) {
  const restaurantTypes = {
    'breakfast': {
      'Budget': { name: 'Local Café', price: '$10-15', cuisine: 'Café' },
      'Medium': { name: 'Artisan Breakfast House', price: '$15-25', cuisine: 'Brunch' },
  ...
  return {
    ...
    bookingLink: `https://www.opentable.com/s?term=${encodeURIComponent(destination + ' restaurant')}`,
    mapsLink: getGoogleMapsLink(restaurant.name, destination),
    menuHighlights: getMealHighlights(mealType),
    reservationRequired: budget !== 'Budget'
  };
}
...
function getHotelAmenities(hotelType) {
  const amenities = {
    'Budget': ['Free WiFi', 'Daily Housekeeping', 'Air Conditioning', '24/7 Reception'],
    'Mid-range': ['Free WiFi', 'Breakfast Included', 'Fitness Center', 'Restaurant', 'Room Service'],
    'Upscale': ['Free WiFi', 'Breakfast Buffet', 'Spa', 'Pool', 'Fine Dining', 'Bar/Lounge'],
    'Luxury': ['Premium WiFi', 'Butler Service', 'Michelin Restaurant', 'Infinity Pool', 'Spa & Wellness'],
    'Ultra-Luxury': ['Fiber WiFi', '24/7 Butler', 'Multiple Restaurants', 'Private Beach', 'Helicopter Pad']
  };
  return amenities[hotelType] || amenities['Mid-range'];
}

function getHotelsForBudget(budget, destination) {
  const hotels = {
    'Budget': [
      {
        name: `${destination} Budget Inn`,
        rating: "3.8",
        price: "$45/night",
        description: "Clean, comfortable budget accommodation.",
        address: `Downtown ${destination}`,
        type: "Budget",
        image: getHotelImageUrl('', destination, 'Budget'),
        mapsLink: getGoogleMapsLink('Budget Inn', destination),
        bookingLink: getBookingLink('Budget Inn', destination),
        roomTypes: ['Standard Room', 'Twin Room'],
        amenities: getHotelAmenities('Budget'),
        ...
        nearbyAttractions: [`${destination} City Center`],
        distanceFromCenter: '0.5 km',
      ...
      },
      {
        ...
        type: "Mid-range",
        image: getHotelImageUrl('', destination, 'Mid-range'),
        mapsLink: getGoogleMapsLink('Comfort Hotel', destination),
        bookingLink: getBookingLink('Comfort Hotel', destination),
        ...
        contact: {
          ...
          email: `reservations@comfort.com`,
          website: `https://www.comforthotel.com`
        },
        nearbyAttractions: [`${destination} Museum`],
        distanceFromCenter: '0.3 km',
        coordinates: getLocationCoordinates()
      }
    ],
    'Medium': [
      {
        ...
        mapsLink: getGoogleMapsLink('Plaza Hotel', destination),
        bookingLink: getBookingLink('Plaza Hotel', destination),
        roomTypes: ['Superior Room', 'Executive Suite'],
        amenities: getHotelAmenities('Mid-range'),
        checkIn: '3:00 PM',
        checkOut: '12:00 PM',
        contact: {
          phone: `+1 (555) 345-6789`,
          email: `stay@plaza.com`,
          website: `https://www.plazahotel.com`
        },
        nearbyAttractions: ['Shopping Mall'],
        distanceFromCenter: '0.2 km',
        coordinates: getLocationCoordinates()
      },
      {
        ...
        rating: "4.5",
        price: "$150/night",
        description: "Upscale accommodation with pool and spa.",
        address: `Prime ${destination}`,
        type: "Upscale",
        image: getHotelImageUrl('', destination, 'Upscale'),
        mapsLink: getGoogleMapsLink('Grand Hotel', destination),
        bookingLink: getBookingLink('Grand Hotel', destination),
        roomTypes: ['Deluxe Ocean View', 'Junior Suite'],
        amenities: getHotelAmenities('Upscale'),
        checkIn: '3:00 PM',
        checkOut: '1:00 PM',
        contact: {
          phone: `+1 (555) 456-7890`,
          email: `concierge@grand.com`,
          website: `https://www.grandhotel.com`
        },
        nearbyAttractions: ['Waterfront'],
        distanceFromCenter: '0.4 km',
        coordinates: getLocationCoordinates()
      }
    ],
    'Luxury': [
      {
        ...
        rating: "4.7",
        price: "$250/night",
        description: "5-star luxury with premium amenities.",
        address: `Premium ${destination}`,
        type: "Luxury",
        image: getHotelImageUrl('', destination, 'Luxury'),
        mapsLink: getGoogleMapsLink('Luxury Suites', destination),
        bookingLink: getBookingLink('Luxury Suites', destination),
        roomTypes: ['Luxury Suite', 'Royal Suite'],
        amenities: getHotelAmenities('Luxury'),
        checkIn: '2:00 PM',
        checkOut: '2:00 PM',
        contact: {
        ...
        nearbyAttractions: ['Private Beach'],
        distanceFromCenter: '2 km',
        coordinates: getLocationCoordinates()
      },
      {
        ...
        address: `Elite ${destination}`,
        type: "Ultra-Luxury",
        image: getHotelImageUrl('', destination, 'Ultra-Luxury'),
        mapsLink: getGoogleMapsLink('Royal Palace Hotel', destination),
        bookingLink: getBookingLink('Royal Palace Hotel', destination),
        roomTypes: ['Palace Suite', 'Royal Suite'],
        amenities: getHotelAmenities('Ultra-Luxury'),
        checkIn: 'Flexible',
        checkOut: 'Flexible',
        contact: {
          phone: `+1 (555) 678-9012`,
          email: `butler@royalpalace.com`,
          website: `https://www.royalpalacehotel.com`
        },
        nearbyAttractions: ['Historic District'],
        distanceFromCenter: '1.5 km',
        coordinates: getLocationCoordinates()
      }
    ]
...
function generateFallbackItinerary(destination, days, budget, travelers, interests) {
  ...
  for (let day = 1; day <= parseInt(days); day++) {
    dailyPlan.push({
      ...
      title: day === 1 ? 'Arrival & Exploration' : 
             day === parseInt(days) ? 'Final Day & Departure' : 
             `Day ${day}: Exploring ${destination}`,
      hotels: getHotelsForBudget(budget, destination),
      activities: [
        {
          time: "9:00 AM",
          activity: "Breakfast & Morning Start",
          description: `Begin your day with a delicious breakfast.`,
          estimatedCost: budget === 'Budget' ? "$12" : budget === 'Medium' ? "$20" : "$35",
          location: `${destination} Breakfast District`,
          restaurant: getRestaurantForActivity(destination, 'breakfast', budget),
          type: 'meal',
          coordinates: getLocationCoordinates()
        },
        {
          time: "10:30 AM",
          activity: "Morning Exploration",
          description: `Discover the best of ${destination}.`,
          estimatedCost: "$35",
          location: `${destination} City Center`,
          type: 'activity',
          mapsLink: getGoogleMapsLink('City Center', destination),
        ...
        },
        {
          time: "1:00 PM",
          activity: "Authentic Local Lunch",
          description: `Experience authentic ${destination} flavors.`,
          estimatedCost: budget === 'Budget' ? "$15" : budget === 'Medium' ? "$28" : "$50",
          location: "Restaurant District",
          restaurant: getRestaurantForActivity(destination, 'lunch', budget),
          type: 'meal',
          coordinates: getLocationCoordinates()
        },
        {
        ...
        },
        {
          time: "7:00 PM",
          activity: "Sunset Dinner & Evening",
          description: `Watch the sunset and enjoy dinner.`,
          estimatedCost: budget === 'Budget' ? "$25" : budget === 'Medium' ? "$45" : "$85",
          location: "Waterfront District",
          restaurant: getRestaurantForActivity(destination, 'dinner', budget),
          type: 'meal',
          mapsLink: getGoogleMapsLink('Waterfront', destination),
          coordinates: getLocationCoordinates()
        }
  ...
  return {
    tripTitle: `${days}-Day ${destination} Adventure`,
    destinationImage: `https://source.unsplash.com/1600x900/?${encodeURIComponent(destination)},travel`,
    overview: `Experience ${destination} over ${days} unforgettable days!`,
    dailyPlan: dailyPlan,
    budgetBreakdown: {
...
app.get('/api/places-autocomplete', async (req, res) => {
  try {
    ...
    console.log('Places search for:', input);
    ...
    console.log('Places found:', response.data.features?.length || 0);
    res.json(response.data);
  } catch (error) {
