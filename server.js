...
// ============================================
// CORS CONFIGURATION - ALLOWS ALL VERCEL URLS
// ============================================
app.use(cors({
  origin: function (origin, callback) {
    ...
    // Allow any origin if FRONTEND_URL is set to *
    ...
    // If none of the above, still allow (for development)
    return callback(null, true);
  },
  credentials: true,
...
app.get('/', (req, res) => {
  res.json({ 
    ...
    endpoints: {
      health: '/api/health',
      generateTrip: 'POST /api/generate-trip',
      placesAutocomplete: 'GET /api/places-autocomplete?input=city'
    }
  });
});
...
function generateFallbackItinerary(destination, days, budget, travelers, interests) {
    ...
    dailyPlan.push({
      ...
      activities: [
        ...
        },
        {
          time: "10:30 AM",
          activity: "Morning Exploration",
          description: `Discover the best of ${destination}. Focus on ${interests || 'sightseeing'} and explore iconic landmarks. Visit museums, historic sites, or local markets.`,
          estimatedCost: "$35",
          location: `${destination} City Center`,
          type: 'activity',
          mapsLink: getGoogleMapsLink('City Center', destination),
        ...
        },
        {
          time: "3:00 PM",
          activity: "Afternoon Adventure",
          description: `Continue your exploration with activities related to ${interests || 'local culture'}. Visit museums, parks, galleries, or take a guided tour.`,
          estimatedCost: "$40",
          location: "Cultural District",
          type: 'activity',
          mapsLink: getGoogleMapsLink('Cultural District', destination),
  ...
  return {
    ...
    overview: `Experience the magic of ${destination} over ${days} unforgettable days! This ${budget.toLowerCase()}-friendly itinerary is perfectly crafted for ${travelers} traveler(s) who love ${interests || 'exploring new places'}. We've selected the best hotels and activities to make your trip memorable.`,
    dailyPlan: dailyPlan,
    budgetBreakdown: {
      accommodation: `$${accommodationCost} (${days} nights, ${budget} tier hotels)`,
      food: `$${Math.round(totalCost * 0.30)}`,
...
// Places autocomplete using Geoapify
app.get('/api/places-autocomplete', async (req, res) => {
  try {
    const { input } = req.query;
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`\n🔑 API Status:`);
  console.log(`   ${process.env.GEMINI_API_KEY ? '✅' : '❌'} Gemini AI: ${process.env.GEMINI_API_KEY ? 'Configured' : 'Missing'}`);
  console.log(`   ${process.env.GEOAPIFY_API_KEY ? '✅' : '❌'} Geoapify: ${process.env.GEOAPIFY_API_KEY ? 'Configured' : 'Missing'}`);
  ...
  console.log(`\n🎯 Available Endpoints:`);
  console.log(`   POST /api/generate-trip - Generate trip itinerary`);
  console.log(`   GET  /api/places-autocomplete - Search cities`);
